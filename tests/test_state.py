...
async def test_transfer(node_provider):
    ...
    async with client1.connect(auth_token1) as api:
        ...
        assert (
            await api.state.balance_for_address(address2)
        ).amount == new_balance2.amount + 20
        assert (
            await api.state.balance_for_address(address3)
        ).amount == new_balance3.amount + 20
...
async def test_undelegate(node_provider, validator_addresses):
    ...
    try:
        async with client.connect(auth_token) as api:
            ...
            assert len(query_unbonding1.unbond.entries) + 1 == len(
                query_unbonding2.unbond.entries
            )

            await api.state.cancel_unbonding_delegation(
                validator1, 3000, undelegate.height
            )
            ...
            with pytest.raises(ValueError):
                await api.state.undelegate(
                    validator1,
                    query_delegation.delegation_response.balance.amount + 1000,
                )
            await api.state.undelegate(
    ...
    try:
        async with client.connect(auth_token) as api:
            ...
            assert (
                ...
                == amount_validator1_before_delegation + 10000
            )
            ...
            assert (
                ...
                == amount_validator1_before_delegation + 1
            )
            assert (
                (
                    await api.state.query_delegation(validator2)
                ).delegation_response.balance.amount
                == amount_validator2_before_delegation + 9999
            )
