...
async def test_header(auth_token):
    ...
    async with client.connect(auth_token) as api:
        ...
        assert local_height <= int(network_head['header']['height'])
        ...
        heads = await api.header.get_range_by_height(await api.header.get_by_height(local_height - 5), local_height)
        assert len(heads) == 4
