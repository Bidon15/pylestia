...
async def test_e2e_blob_submission():
    ...
    async with client.connect(auth_token) as api:
        ...
        assert retrieved_blobs[0].data == hello_world_data, "Retrieved blob data should match"
        assert retrieved_blobs[0].namespace == namespace, "Retrieved namespace should match"
...
async def test_e2e_blob_with_signer():
    ...
    test_signer = b"celestia1testaddress"  # In a real scenario, this would be a valid address
    ...
    async with client.connect(auth_token) as api:
        ...
        try:
            ...
            print(f"Note: Signer verification failed: {e}. This is expected if the node doesn't support signers.")
